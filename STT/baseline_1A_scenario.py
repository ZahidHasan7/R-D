"""
baseline_1A_scenario.py — Approach 1A: Raw Baseline (Scenario-Level)
===================================================================
Transcribes full scenario audio files and compares against concatenated transcripts.
"""
...
def clean(text: str) -> str:
    ...
    # Remove punctuation for better WER matching
    text = re.sub(r"[।!?,;:\.\'\"\-—]", " ", text)
...
for name, data in scenarios.items():
    ...
    if not audio_path.exists():
        print(f"  [SKIP] {name}: Audio not found at {audio_path}")
        continue
    ...
    # transcribe
    print(f"  Transcribing {name} ({audio_path.name})...")
    result = model.transcribe(str(audio_path), language="bn")
...
log_entry = {
    ...
    "approach": "1A_RawBaseline_FullScenario",
    "metrics": metrics,
    "scenarios": results
}
